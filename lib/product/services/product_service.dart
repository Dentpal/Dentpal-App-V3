...
class ProductService {
  ...
  Future<Map<String, dynamic>> getProductsPaginated({
    ...
    try {
      ...
      DocumentSnapshot? lastVisibleDocument = querySnapshot.docs.isNotEmpty
      ...
      AppLogger.d(
        'Fetching variations for ${filteredProducts.length} products in parallel...',
      );
      ...
      List<List<ProductVariation>> allVariations = await Future.wait(
        variationFutures,
      );
    ...
    try {
      ...
      if (!sellerDoc.exists) {
        return {
          'isSeller': false,
          'message':
              'Seller profile not found. Please contact support to set up your seller account.',
          'sellerId': null,
        };
      }
      ...
      try {
        ...
        AppLogger.d(
          'User collection check failed, but continuing with Seller validation: $e',
        );
      }
  ...
  Future<Map<String, dynamic>> addProduct(
    ...
    try {
      ...
      for (var variationForm in variations) {
        DocumentReference variationRef = productRef
            .collection('Variation')
            .doc();
      ...
      return {'success': false, 'message': 'Error: $e', 'productId': null};
    }
  }
  ...
  Future<Map<String, dynamic>> updateProduct(
    ...
    try {
      ...
      if (!sellerStatus['isSeller']) {
        return {'success': false, 'message': sellerStatus['message']};
      }
      ...
      if (isDraft != null) {
        ...
        updateData['isActive'] =
            !isDraft; // If it's a draft, set isActive to false
      }
      ...
      QuerySnapshot existingVariations = await productRef
          .collection('Variation')
          .get();
      ...
      for (int index = 0; index < variations.length; index++) {
        ...
        if (existingVariationId != null &&
            existingVariationsMap.containsKey(existingVariationId)) {
          // Update existing variation
          await productRef
              .collection('Variation')
              .doc(existingVariationId)
              .update({
                'name': variationForm.name,
                'imageURL': variationForm.imageURL,
                'price': variationForm.price, // Price already includes VAT
  ...
  Future<List<Product>> getProductsBySeller(String sellerId) async {
    try {
      ...
      AppLogger.d(
        'ProductService: Simple query successful with ${querySnapshot.docs.length} documents',
      );

      AppLogger.d(
        'ProductService: Query returned ${querySnapshot.docs.length} documents',
      );
      ...
      for (var doc in querySnapshot.docs) {
        try {
          ...
          if (!product.isActive) {
            AppLogger.d(
              'ProductService: Skipping inactive product ${product.name}',
            );
            continue;
          }
          ...
          AppLogger.d(
            'ProductService: Found ${variationsSnapshot.docs.length} variations for ${product.name}',
          );
          ...
          AppLogger.d(
            'ProductService: Added product ${product.name} to results',
          );
        } catch (productError) {
          AppLogger.d(
            'ProductService: Error processing product document ${doc.id}: $productError',
          );
        }
      }

      AppLogger.d(
        'ProductService: Final result - Fetched ${products.length} active products for seller $sellerId',
      );
      return products;
    } catch (e) {
      AppLogger.d(
        'ProductService: Error fetching products for seller $sellerId: $e',
      );
